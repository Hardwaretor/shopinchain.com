...
m_cfg.set("physics_uranium_path", "blend4web_ce/dist/uranium/")
...
function init_cb(canvas_elem, success) {
    ...
    if (!success) {
        console.log("ShopInChain init failure");
        return;
    }

    m_preloader.create_preloader({
        container_color:"#000", // background color of the container 
        bar_color:"#ff8400", // background color of the bar 
...
function enable_fullscreen_cb() {
    console.log("ShopInChain in VR-mode.");
    _switch_vr_button.innerText = "Back to Home";
...
function disable_fullscreen_cb() {
    ...
    console.log("ShopInChain in One Eye Mode.");
    _switch_vr_button.innerText = m_input.can_use_device(m_input.DEVICE_HMD) ?
            "VR Shop": "VR Shop is not avaliable";
}
...
function enable_hmd_configurator(controls_container) {
    ...
    if (hmd_type & (m_input.HMD_NON_WEBVR | m_input.HMD_WEBVR_MOBILE |
        ...
        var hmd_conf_button = create_button("HMD setup.");
...
function load() {
    m_data.load("blend4web_ce/projects/shop/vr.json", load_cb, preloader_cb);
}
...
function load_cb(data_id, success) {
    ...
    if (!success) {
        console.log("ShopInChain load failure");
        return;
    }
